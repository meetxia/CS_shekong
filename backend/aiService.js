...
function buildPrompt(report, answers, basicInfo) {
  ...
  return `你是一位温暖、善解人意的心理陪伴者，就像用户最信任的朋友或贴心的心理咨询师。你的任务是用最温暖、最通俗的语言，帮助用户理解自己的社交焦虑模式。
...
async function generateAIAnalysis(report, answers, basicInfo) {
  ...
  try {
    ...
    console.log(`🤖 调用 ${config.provider} AI API...`);
